...
main() {
    ...
    if [[ ! -f /etc/autoglm/frontend.env ]]; then
        install_managed_file "${SCRIPT_DIR}/frontend.env.example" "/etc/autoglm/frontend.env" root root 640
    fi
    ...
    info "Env file    : /etc/autoglm/frontend.env"
