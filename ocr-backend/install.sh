...
main() {
    ...
    ensure_directory "/etc/autoglm" root root 755
    ...
    if [[ ! -f /etc/autoglm/ocr-backend.env ]]; then
        install_managed_file "${SCRIPT_DIR}/ocr-backend.env.example" "/etc/autoglm/ocr-backend.env" root root 640
    fi
    ...
    info "Env file    : /etc/autoglm/ocr-backend.env"
