...
Edit `/etc/autoglm-ocr/ocr-backend.env`:
...
`setup_tls.sh` and `import_tls_cert.sh` manage the backend API HTTPS certificate in `[api_tls]`.

If the backend needs a separate outbound client certificate for frontend polling or upload targets,
configure `[client_tls]` with a different cert/key pair.
