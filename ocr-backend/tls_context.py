...
def load_tls_settings(cfg: configparser.ConfigParser, section: str) -> tuple[bool, Path | None, Path | None, str]:
    enabled = cfg.getboolean(section, "enabled", fallback=False)
    cert_value = cfg.get(section, "cert_file", fallback="").strip()
    key_value = cfg.get(section, "key_file", fallback="").strip()
    min_tls_version = cfg.get(section, "min_tls_version", fallback="TLSv1.2").strip() or "TLSv1.2"
