...
class SyncCoordinator:
    ...
    def start(self) -> None:
        ...
        if self.config.api_enabled:
            ...
            if self.config.api_tls_enabled:
                context = build_server_ssl_context(
                    tls_enabled=True,
                    cert_file=self.config.api_tls_cert_file,
                    key_file=self.config.api_tls_key_file,
                    min_tls_version=self.config.api_tls_min_tls_version,
                )
                if context is None:
    ...
    def build_health(self, base_url: str) -> dict[str, Any]:
        ...
        return {
            ...
            "sync_enabled": self.config.enabled,
            "tls_enabled": self.config.api_tls_enabled,
            "runtime": status,
        }
...
def main() -> None:
    ...
    logger.info("API TLS enabled   : %s", config.api_tls_enabled)
