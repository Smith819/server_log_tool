...
Notes:
  The same certificate is used for the backend HTTPS API and optional outbound client auth.
EOF
}
...
main() {
    ...
    case "$mode" in
        ...
        self-signed)
            ...
            info "Generating OCR backend certificate for ${identity}..."
            generate_self_signed_pair "$DEST_CERT" "$DEST_KEY" "$SERVICE_GROUP" "$identity" "serverAuth,clientAuth" "$(san_for_identity "$identity")"
            patch_config "$DEST_CERT" "$DEST_KEY"
            restart_component_services
            info "OCR backend certificate installed at ${DEST_CERT}"
            ;;
        letsencrypt)
            require_root
            ensure_python3
            ...
            restart_component_services
            info "OCR backend Let's Encrypt certificate synced to ${CERT_DIR}"
            ;;
        *)
